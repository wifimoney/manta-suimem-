module tide::vault {
    ...
    public fun register_memory(registry: &mut VaultRegistry, memory: &MemoryObject, ctx: &mut TxContext) {
...
public fun deregister_memory(registry: &mut VaultRegistry, memory: &MemoryObject, ctx: &mut TxContext) {
    let owner = tx_context::sender(ctx);
    assert!(owner == memory::get_owner(memory), 0);
    let memory_id = object::id_to_address(&memory::get_id(memory));
