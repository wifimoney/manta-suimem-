...
export class TideClient {
  ...
  constructor(config: TideClientConfig) {
    this.network = config.network;
    this.packageId = PACKAGE_IDS[config.network];

    if (!this.packageId) {
      throw new Error(`Tide not deployed on ${config.network}`);
    }
  ...
  // ============ Vault Functions (Seal Encryption) ============
  registerMemory(registryId: string, memoryId: string): Transaction {
    const tx = new Transaction();

    tx.moveCall({
      target: `${this.packageId}::${MODULES.VAULT}::register_memory`,
      arguments: [tx.object(registryId), tx.object(memoryId)],
    });
    return tx;
  }

  deregisterMemory(registryId: string, memoryId: string): Transaction {
    const tx = new Transaction();

    tx.moveCall({
      target: `${this.packageId}::${MODULES.VAULT}::deregister_memory`,
      arguments: [tx.object(registryId), tx.object(memoryId)],
    });
    return tx;
  }

  // ============ Extended Functions (Walrus Blob Storage) ============

  attachBlob(registryId: string, memoryId: string, blobId: string, sizeBytes: number): Transaction {
    const tx = new Transaction();


    tx.moveCall({
      target: `${this.packageId}::${MODULES.EXTENDED}::attach_blob`,
      arguments: [tx.object(registryId), tx.object(memoryId), tx.pure('address', blobId), tx.pure('u64', sizeBytes.toString())],
    });
    return tx;
  }

  detachBlob(registryId: string, memoryId: string, blobId: string): Transaction {
    const tx = new Transaction();

    tx.moveCall({
      target: `${this.packageId}::${MODULES.EXTENDED}::detach_blob`,
      arguments: [tx.object(registryId), tx.object(memoryId), tx.pure('address', blobId)],
    });
    return tx;
  }
  ...
  parseEvents(response: SuiTransactionBlockResponse): TideEvent[] {
