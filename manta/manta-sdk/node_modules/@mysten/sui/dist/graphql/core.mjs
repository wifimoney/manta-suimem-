...
import { parseTransactionEffectsBcs } from "../client/utils.mjs";
...
var GraphQLCoreClient = class extends CoreClient {
	...
	resolveTransactionPlugin() {
		...
		return async function resolveTransactionData(transactionData, options, next) {
			...
			handleGraphQLErrors(errors);
			if (data?.simulateTransaction?.error) throw new Error(`Transaction resolution failed: ${data.simulateTransaction.error}`);
...
function parseGraphQLExecutionError(executionError) {
	...
	if (mapGraphQLExecutionErrorKind(executionError) === "MoveAbort" && executionError?.abortCode != null) return {
		$kind: "MoveAbort",
		message,
		MoveAbort: {
