...
import { MantaClient, SchemaType, Permissions } from './src';
...
async function main() {
    console.log('🌊 Starting Manta E2E Test...');

    // 1. Setup Client & Signer
    const PRIVATE_KEY = process.env.MANTA_PRIVATE_KEY;
    let keypair;

    if (!PRIVATE_KEY) {
        console.log('⚠️ No MANTA_PRIVATE_KEY found. Generating ephemeral wallet...');
        ...
        try {
            ...
            console.error('❌ Faucet failed. Please set MANTA_PRIVATE_KEY or try again later.');
            console.error(e);
            process.exit(1);
        }
    } else {
    ...
    const manta = new MantaClient({ network: 'testnet', client });
    ...
    const appendTx = manta.append(memoryId, logMessage);
    ...
    const createSharedTx = manta.createSharedSemanticMemory();
    ...
    const val1 = "dark";
    const updateTx = manta.update(sharedMemoryId, key1, val1);
    await client.signAndExecuteTransaction({
    ...
    const delegateTx = manta.delegateAppend(sharedMemoryId, keypair.toSuiAddress());
    ...
    const failTx = manta.capUpdate(sharedMemoryId, capId, "should", "fail");
    try {
        await client.signAndExecuteTransaction({
